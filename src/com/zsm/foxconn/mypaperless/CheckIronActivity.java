...
public class CheckIronActivity extends BaseActivity implements OnClickListener {

	private UserBean user;
	HttpStart start;
	private Context context = CheckIronActivity.this;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_check_iron);
		...
		radio.setOnCheckedChangeListener(new OnCheckedChangeListener() {
			...
			public void onCheckedChanged(RadioGroup group, int checkedId) {
				...
				// alert [toolName]改为全局变量
				toolName = ra.getText().toString();
				if (toolName.equals("风枪")) {
					bianhao(toolName);
					AlertDialog alert = new AlertDialog.Builder(context)
							.create();
					alert.setIcon(R.drawable.img8);
					alert.setTitle("系統提示：");
					alert.setMessage("您是否开始点检热风枪？");
					alert.setButton(DialogInterface.BUTTON_NEGATIVE, "確定",
							new DialogInterface.OnClickListener() {
								...
								public void onClick(DialogInterface dialog,
									...
									// radio.getChildAt(1).setClickable(true);
									expListView = (ExpandableListView) findViewById(R.id.LTlaptop_list);
									r = "熱風槍點檢項目";
									start.getServerData(0,
											MyConstant.GET_PRONAME, mfg,
											reportNum, r);

									Log.i("", reportNum + "   ---  " + RNO);
								}
							});
					alert.setButton(DialogInterface.BUTTON_POSITIVE, "取消",
							new DialogInterface.OnClickListener() {
								// 為二級標題中單選按鈕變動并改變第二個文本框賦值（EdText2）
								@Override
								public void onClick(DialogInterface dialog,
										int which) {
									((RadioButton) radio.getChildAt(0))
											.setChecked(true);
								}
							});
					alert.show();
	...
	protected void findViewById() {
		...
		layout_check_aoi = (LinearLayout) findViewById(R.id.activity_check_iron);
	}
	...
	public void onClick(View v) {
		...
		case R.id.select:
			if (autoTV.getText().toString().length() > 0) {
				...
				Toast.makeText(context, "请先选择需点检编号的类型", Toast.LENGTH_LONG)
						.show();
			}
			break;
		...
		case R.id.btn_submit:
			if (autoTV.length() > 0) {
				...
				alert.setButton(DialogInterface.BUTTON_NEGATIVE,
						"是的,已審核,繼續提交！", new DialogInterface.OnClickListener() {
				...
				alert.setButton(DialogInterface.BUTTON_POSITIVE,
						"否，立即交由當前線長審核！", new DialogInterface.OnClickListener() {
							@Override
							public void onClick(DialogInterface dialog,
									int which) {
				...
				Toast.makeText(context, "请先选择需要提交的类型", Toast.LENGTH_LONG)
						.show();
			}
			break;
		}
	...
	public void bianhao(String toolName) {
		try {
			start.getServerData(0, MyConstant.GET_TOOLNO, mfg, toolName);
			ad = new ArrayAdapter<String>(context,
					android.R.layout.simple_dropdown_item_1line, ToolNos);
			autoTV.setAdapter(ad); // 把带出来定的LT编号放进数据源
		} catch (Exception e) {
	...
	private Handler handler = new Handler() {
		public void handleMessage(Message msg) {
			...
			for (String key : keySet) // 键值对
			{
				if (key.equals(MyConstant.GET_SHIFT)) {
				...
				} else if (key.equals(MyConstant.GET_TOOLNO)) {
					result = new ArrayList<String>();
					if (ToolNos.size() > 0) {
						ToolNos.clear();
					}
					...
					if (result.get(0)
							.equalsIgnoreCase(MyConstant.GET_FLAG_TRUE)) {
						for (int i = 1; i < result.size(); i++) {
							ToolNos.add(result.get(i).toString());
						}
						String Tool = "";
						for (int i = 0; i < ToolNos.get(1).length(); i++) {
							char T = ToolNos.get(1).charAt(i);
							if (T != '-') {
								Tool += T;
							} else {
								break;
					...
					checkLINE();
				} else if (key.equals(MyConstant.GET_PRONAME)) {
