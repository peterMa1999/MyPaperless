...
/**
 * 補點檢(與點檢ECheckActivity.java類似)
 * 
 * @author F1330297
 * 
 */
public class Check_Up_ECheckActivity extends BaseActivity implements
		OnClickListener {

	private UserBean user;
	HttpStart start;
	private Context context = Check_Up_ECheckActivity.this;
	...
	private String is_input_order, is_usercheck, time, PDteam, CheckUpTime;
	...
	private Handler handler = new Handler() {
		public void handleMessage(Message msg) {
			...
			for (String key : keySet) // 键值对
			{
				...
				} else if (key.equals(MyConstant.SAVE_CHECKREPORT_CHECK_UP)) {
					result = new ArrayList<String>();
					result = msg.getData().getStringArrayList(key);
					if (result.get(0)
							.equalsIgnoreCase(MyConstant.GET_FLAG_TRUE)) {
						if (!tijiao_way_flag) {
							...
							start.getServerData(0,
									MyConstant.SAVE_CHECK_BASE_INFO_CHECK_UP,
				...
				} else if (key.equals(MyConstant.SAVE_CHECK_BASE_INFO_CHECK_UP)) //
				{
					result = msg.getData().getStringArrayList(key);
					if (result.get(0)
							.equalsIgnoreCase(MyConstant.GET_FLAG_TRUE)) {
						...
						finalDb.deleteByWhere(ChildModel.class, "Logonname='"
								...
								+ "' and line_name='" + mline + "'");
						Check_Up_ECheckActivity.this.finish();
					} else if (result.get(0).equalsIgnoreCase(
							MyConstant.GET_FLAG_NULL)) {
						Toast.makeText(context, "提交PD点检基本信息失败", 0).show();
					}
				} else if (key.equals(MyConstant.SAVE_CHECK_PERSON_CHECK_UP)) {
					result = new ArrayList<String>();
					result = msg.getData().getStringArrayList(key);
					if (result.get(0)
							.equalsIgnoreCase(MyConstant.GET_FLAG_TRUE)) {
						start.getServerData(0,
								MyConstant.SAVE_CHECKREPORT_CHECK_UP, RNO,
								reportNum, mfg, floor, mline, CheckUpTime,
								user.getLogonName());
					}
					if (result.get(0)
							.equalsIgnoreCase(MyConstant.GET_FLAG_NULL)) {
	...
	private void popsindows() {
		...
		window.showAtLocation(
				Check_Up_ECheckActivity.this.findViewById(R.id.btn_submit),
				Gravity.BOTTOM, 0, 0);
		...
		OK.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (liststr2.size() > 0) {
					...
					if (checkFlag) {
						start.getServerData(0,
								MyConstant.SAVE_CHECK_PERSON_CHECK_UP, RNO,
								personNameString, user.getLogonName()
										.toString().trim(), CheckUpTime);
						// --------------------------------------勾选生成异常提交popwindow-----------------------------
	...
	public void QR() {
		Intent openCameraIntent = new Intent(Check_Up_ECheckActivity.this,
	...
	private void AbnormalPopwindow() {
		...
		popuWindow.showAtLocation(
				Check_Up_ECheckActivity.this.findViewById(R.id.center_layout),
				Gravity.CENTER, 0, 0);
