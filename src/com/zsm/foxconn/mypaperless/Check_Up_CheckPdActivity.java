...
/**
 * 補點檢(與點檢CheckPdActivity.java類似)
 * 
 * @author F1330297
 * 
 */
public class Check_Up_CheckPdActivity extends BaseActivity {
	private ImageButton imageButtonUP, imageButtonUP2, submitbutton;
	private Button scanQR;
	private Spinner beizhuSpinner, questionSpinner;
	private Context context = Check_Up_CheckPdActivity.this;
	...
	private String report_Num = null, reportName = null, selectLine = null,
			flagfloor = null, CheckUpTime = null;
	List<List<Object>> saveData = null;
	private String section = null;
	String Check_Result = "";
	private String beizhuString, questionString, radioString;
	String personNameString = null;
	private boolean isOk = false, isnull = true;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		report_Num = intent.getStringExtra("reportid");
		reportName = intent.getStringExtra("reportname");
		selectLine = intent.getStringExtra("linename");
		flagfloor = intent.getStringExtra("floorName");
		section = intent.getStringExtra("section");
		CheckUpTime = intent.getStringExtra("CheckUpTime");
		CheckLogin();
		findViewById();
	...
	public void QR() {
		Intent openCameraIntent = new Intent(Check_Up_CheckPdActivity.this,
				CaptureActivity.class);
	...
	private Handler handler = new Handler() {
		public void handleMessage(Message msg) {
			...
			for (String key : keySet) // 键值对
			{
				...
				if (key.equals(MyConstant.SAVE_CHECK_BASE_INFO_CHECK_UP)) //
				{
					result = msg.getData().getStringArrayList(key);
					if (result.get(0)
							.equalsIgnoreCase(MyConstant.GET_FLAG_TRUE)) {
				...
				if (key.equals(MyConstant.SAVE_CHECK_RESULT)) //
				{
					...
					if (result.get(0)
							.equalsIgnoreCase(MyConstant.GET_FLAG_TRUE)) {
						Toast.makeText(context, "提交PD点检结果成功", 5000).show();
						deleteLine();
					} else if (result.get(0).equalsIgnoreCase(
							MyConstant.GET_FLAG_NULL)) {
						Toast.makeText(context, "提交PD点检结果失败...", 0).show();
					}
				}
				if (key.equals(MyConstant.SAVE_CHECK_REPORT_CHECK_UP)) //
				{
					result = msg.getData().getStringArrayList(key);
					if (result.get(0)
							.equalsIgnoreCase(MyConstant.GET_FLAG_TRUE)) {
				...
				if (key.equals(MyConstant.SAVE_CHECK_PERSON_CHECK_UP)) //
				{
					result = msg.getData().getStringArrayList(key);
					if (result.get(0)
							.equalsIgnoreCase(MyConstant.GET_FLAG_TRUE)) {
						...
						// 保存PD点检基本信息;
						start.getServerData(0, MyConstant.SAVE_CHECK_BASE_INFO_CHECK_UP,
								totalDate, CheckUpTime);
					} else if (result.get(0).equalsIgnoreCase(
							MyConstant.GET_FLAG_NULL)) {
							...
							.setPositiveButton(
									...
									new DialogInterface.OnClickListener() {
										public void onClick(
											...
											start.getServerData(
													0,
													MyConstant.SAVE_CHECK_PERSON_CHECK_UP,
													RNO_textView.getText()
															.toString(),
													personNameString, userBean
															.getLogonName()
															.toString().trim(),
													CheckUpTime);
